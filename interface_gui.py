...
"""
Drag-&-Drop GUI for
calculos_rt54_rg4115.py  (RT-54 / RG-4115 accounting entries)
Flet 0.22+
"""
import io
import logging
...
class LogPipe(io.StringIO):
    """Tiny helper: redirect logging -> Flet Text control"""
...
def main(page: ft.Page):
    ...
    # ---- state -------------------------------------------
    df_asientos = None          # resulting DataFrame
    drop_border = ft.BorderSide(2, ft.colors.BLUE_GREY_400)
    drop_bg = ft.colors.BLUE_GREY_50

    # ---- widgets -----------------------------------------
    drop_zone = ft.Container(
        content=ft.Column(
            [
                ft.Icon(ft.icons.CLOUD_UPLOAD, size=48, color=ft.colors.BLUE_GREY_400),
                ft.Text("Arrastrá el archivo CSV/Excel aquí", size=16, weight=ft.FontWeight.BOLD),
            ],
        ...
        height=180,
        bgcolor=drop_bg,
        border=ft.border.all(drop_border),
        border_radius=12,
    ...
    def on_drag_enter(e):
        drop_zone.bgcolor = ft.colors.BLUE_GREY_100
        drop_zone.update()

    def on_drag_leave(e):
        drop_zone.bgcolor = drop_bg
        drop_zone.update()
    ...
    def on_drop(e):
        ...
        if not e.data:
            return
        # Flet passes file names separated by \n
        file_path = e.data.split("\n")[0].strip('"')
